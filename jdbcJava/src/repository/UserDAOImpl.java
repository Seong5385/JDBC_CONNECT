...
public class UserDAOImpl implements Users {
    ...
    public int userAdd(UsersVO vo) {
        ...
        int result = 0;
        try (Connection conn = DriverManager.getConnection(this.url, this.user, this.pw)) {
            String sql = """
                    INSERT INTO persons
                    (userId, userPw, userName, userEmail, phone1, phone2, age, addr1, add2)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """;
    ...
    @Override
    public List<UsersVO> userSearchEmail() throws IOException {
        BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
